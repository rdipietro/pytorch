...
void checkGridSize(THVoidTensor *grid, THVoidTensor *input)
{
  ...
  CHECK_ARG(grid->size[1] == input->size[2]);
  CHECK_ARG(grid->size[2] == input->size[3]);
  CHECK_ARG(grid->size[3] == 2);  
}

void checkIOSize(THVoidTensor *input, THVoidTensor *output)
{
  ...
  for (int i=0; i < 4; ++i) {
    CHECK_ARG(input->size[i] == output->size[i]);
  }
}
    ...
    THVoidTensor* grad_output)
{
  ...
  checkIOSize(input, grad_output);
  checkIOSize(input, grad_input);
